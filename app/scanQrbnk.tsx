...
export default function ScanQR() {
  ...
  const [qrText, setQRText] = useState(""); // สำหรับ web
  // ---------- จบโซน hooks ----------
  ...
  // ---------- Web mode: แสดงแบบฟอร์มสำหรับใส่ QR data ----------
  if (Platform.OS === "web") {
    if (scanned === null) {
      return (
        <View className="flex-1 bg-slate-50 p-6">
          <View className="rounded-xl bg-blue-50 px-4 py-3">
            <Text className="font-lao text-sm text-blue-700">
              💻 ສະແກນ QR ບໍ່ໄດ້ຢູ່ເວັບ — ໃຫ້ແຕ່ລະຕົວ QR data ເທື່ອລະອັນ
            </Text>
          </View>

          <View className="mt-6 gap-4">
            <Text className="font-lao text-base font-semibold text-slate-900">
              ໃສ່ QR code data:
            </Text>
            <TextInput
              placeholder='ຕົວຢ່າງ: [{"staffID":"001","name":"ສະມາຊິກ 1","date":["01/08/2026"]}]'
              value={qrText}
            ...
            <Pressable
              ...
              onPress={() => {
                if (!qrText.trim()) {
                  window.alert("ກະລຸນາໃສ່ QR data");
                  return;
                }
                const parsed = parseQR(qrText);
                if (!parsed) {
                  window.alert("QR ນີ້ບໍ່ແມ່ນຂໍ້ມູນຕາຕະລາງທີ່ຖືກຕ້ອງ");
                  return;
                }
                setScanned(parsed);
              }}
            >
              <Text className="font-lao text-base text-white">ອ່ານ QR</Text>
            </Pressable>
          </View>
        </View>
      );
    ...
    // Web mode: Preview before upload
    return (
      <View className="flex-1 bg-slate-50 p-6">
        ...
        <View className="flex-row gap-3 pb-6 pt-2">
          ...
          >
            <Text className="font-lao text-base text-slate-700">ສະແກນໃໝ່</Text>
          </Pressable>
          <Pressable
